...
class op_mitting extends functionx {
    ...
    public function removeHotel() {
        if (isset($_GET['id']) && !empty($_GET['id'])) {
            $id = $_GET['id'];
            $this->remove('mitting', "room_id='$id'");
            header("Location: mitting.php");
        }
    }
    ...
    public function updateRoom() {
        ...
        $this->update("mitting", $data, "room_id='$id'");
         ...
         foreach ($_FILES['room_pic_gallery']['name'] AS $key => $value) {
          ...
          header("Location: mittting_form.php?id=$id");
